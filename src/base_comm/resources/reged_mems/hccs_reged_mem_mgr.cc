...
HcclResult HccsRegedMemMgr::UnregisterMemory(void* memHandle)
{
    ...
    if (!found) {
        HCCL_ERROR("[%s] addr[%p] size[%zu] memHandle[%p] allRegisteredBuffers_.size[%zu] Not Found",
            __FUNCTION__, addr, static_cast<size_t>(size), memHandle, allRegisteredBuffers_.size());
        return HCCL_E_NOT_FOUND;
    }
    if (!resultPair) {
