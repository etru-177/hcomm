...
> [!CAUTION]注意
> `CcuBuffer` 类有copy / move 构造函数，它们只拷贝 `handle` 字段、不申请新的MS 切片——`CcuBuffer b2 = b1;` 后 `b1` 和 `b2` 指向同一块MS 切片。如需独立的CcuBuffer，必须显式 `CcuBuffer b2;` 默认构造（或用 `Array<CcuBuffer>` 申请物理连续的多块）。
