...
在CCU kernel内从片上内存中读取一个或多个`uint64_t`值写入Variable。
...
| v | 输入/输出 | 目标Variable（重载1，num=1）。运行期从片上内存中addr指向的位置读取8字节写入该Variable。 |
| vArr | 输入/输出 | 目标Variable数组首元素（重载2，num>1）。须通过`ccu::Array<Variable>`申请以保证物理连续。 |
| num | 输入 | 加载的`uint64_t`元素个数（重载2）。须大于0。<br>设定片上内存中addr指向的位置为`mem[addr]`，`num>1`时读取`mem[addr], mem[addr+8], ..., mem[addr+(num-1)*8]`，分别写入`vArr[0], vArr[1], ..., vArr[num-1]`。 |
