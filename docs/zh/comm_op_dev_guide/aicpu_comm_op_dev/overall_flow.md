...
HCCL AICPU通信算子的开发与执行分为Host侧准备和AICPU侧执行两个阶段。Host侧负责解析拓扑、选择算法、申请资源并下发AICPU Kernel；Kernel启动后，AICPU侧才根据资源上下文进行任务编排。因此，**Kernel下发在前，任务编排在后**。
...
sequenceDiagram
    ...
    HCCL_K->>+HCOMM_D: AICPU控制Thread等待Host启动通知
    HCOMM_D-->>-HCCL_K: HcclResult
...
5. **下发Kernel**：Host与AICPU控制Thread建立启动同步关系，然后将AICPU Kernel下发到执行流。
